...
public class SMIMETest {
	...
	public static void setup() throws Exception {
		...
		InputStream privatePem = Thread.currentThread().getContextClassLoader()
	...
	public void testEncryptedGet() throws Exception {
		WebTarget target = client
				.target("http://localhost:8080/encrypted/smime");
		EnvelopedInput input = target.request().get(EnvelopedInput.class);
		System.out.println(input);
		String contendi = (String) input.getEntity(String.class, privateKey,
				cert);
		System.out.println(contendi);
		// Customer cust = (Customer)input.getEntity(Customer.class, privateKey,
		// cert);
	...
	// {
	// WebTarget target =
	// client.target("http://localhost:9095/smime/encrypted");
	...
	// {
	// WebTarget target =
	// client.target("http://localhost:9095/smime/encrypted/signed");
	// EnvelopedInput enveloped = target.request().get(EnvelopedInput.class);
	// SignedInput signed = (SignedInput)enveloped.getEntity(SignedInput.class,
	// privateKey, cert);
